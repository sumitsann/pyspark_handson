...
# Read usdata.csv using dataframe

# usdatacsvdf = spark.read.format("csv").load("usdata.csv")
# usdatacsvdf.show()

# Columns are not looking good. first_name last_name are not in the column name
# We need to use header true in option

usdatacsvdf = spark.read.format("csv").option("header","true").load("usdata.csv")
usdatacsvdf.show()

# Read file4.json using dataframe

file4jsondf = spark.read.format("json").option("header","true").load("file4.json")
file4jsondf.show()

# Read file5.parquet using dataframe

file5parquetdf = spark.read.format("parquet").option("header","true").load("file5.parquet")
file5parquetdf.show()

# Read data.orc using dataframe

dataorcdf = spark.read.format("orc").option("header","true").load("data.orc")
dataorcdf.show()

# Create table named usdatatab
usdatacsvdf.createOrReplaceTempView("usdatatab")

# filter data where state=LA
fildf = spark.sql("SELECT * from usdatatab where state='LA'")
fildf.show()